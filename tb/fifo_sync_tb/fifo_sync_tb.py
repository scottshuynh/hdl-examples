...
async def read_data(dut, num_reads: int):
    ...
    while num_completed_reads < num_reads:
        if dut.IS_FWFT.value:
            if not dut.rd_data_vld_o.value.resolve("zeros"):
                await dut.rd_data_vld_o.value_change
        dut.rd_en_i.value = 1
...
async def test_random_reads_after_writes(dut):
    ...
    await drive_random_writes(dut, write_datas)
    await with_timeout(
        read_to_verify_write_datas(dut, write_datas), 4 * num_writes, "ns"
    )
...
async def test_random_writes_and_reads(dut):
    ...
    await with_timeout(read_task, 32 * num_writes, "ns")
